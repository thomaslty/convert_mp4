...
filetype=".mkv"
subtitle_extension=".srt" # example: ".tc.ass"
...
for file in *$filetype; do
    ...
    ffmpeg -v error -stats -i "$file" -vf "subtitles='$subtitle_filename':fontsdir=$fonts_dir:force_style='fontname=GenSenRounded2 TW R'" -c:v libx264 -c:a aac "$output_dir/$filename.mp4"
done
